...
import {
  KpiCard, Card, SectionTitle, EmptyState, Badge, Avatar,PhotoAvatar
...
export function PredictionPage({ players, venues, teams, apiOk, photoMap = {} }) {
  ...
  // Seed venue + team once lists load
  ...
  const generate = async () => {
    ...
    try {
      const data = await runPrediction({
        player_id: player.ID, spin_type: spinType,
        phase, venue, innings, n_balls: nBalls,
      });
      setPredResult(data);
...
Explain in 3-4 sentences: why this was predicted, key tactical factors, what could make it wrong, and a fantasy cricket recommendation.`;
  ...
  const label = (s) => ({ padding: "9px 12px", borderRadius: 8, border: `1px solid ${G.gray300}`, fontSize: 13, fontFamily: "'Barlow Condensed', sans-serif", outline: "none", width: "100%" });
...
/>
            <div>
              <div style={{ fontWeight: 700, fontSize: 16, color: G.gray800, fontFamily: "'Barlow Condensed', sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        <Card>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <select value={venue} onChange={(e) => setVenue(e.target.value)} style={label()}>
          ...
          <div>
            ...
            <select value={oppTeam} onChange={(e) => setOppTeam(e.target.value)} style={label()}>
      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          <div>
            ...
            <select value={spinType} onChange={(e) => setSpinType(e.target.value)} style={label()}>
          ...
          <div>
            ...
            <select value={innings} onChange={(e) => setInnings(Number(e.target.value))} style={label()}>
          ...
          <div>
            ...
            <select value={nBalls} onChange={(e) => setNBalls(Number(e.target.value))} style={label()}>
      ...
      <button
        onClick={generate}
        disabled={predLoading || !player}
        style={{
          width: "100%", padding: "14px",
          background: !player ? G.gray300 : G.green,
          color: G.white, border: "none", borderRadius: 10,
          fontSize: 16, fontWeight: 700, cursor: !player ? "not-allowed" : "pointer",
          fontFamily: "'Barlow Condensed', sans-serif",
          letterSpacing: 0.5, marginBottom: 20, transition: "all 0.15s",
        }}
      >
        {predLoading ? "⚙ Running Model…" : "✨ Generate AI Prediction"}
      </button>
          ...
          <Card style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <Badge
                label={riskLabel}
                color={riskColor}
                bg={riskColor === G.red ? G.redLight : riskColor === G.amber ? G.amberLight : G.greenLight}
              />
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ fontSize: 36, fontWeight: 800, color: G.gray900, fontFamily: "'Barlow Condensed', sans-serif", minWidth: 60 }}>
                {predResult.confidence}%
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ height: 10, background: G.gray100, borderRadius: 5, overflow: "hidden" }}>
                  <div style={{
                    width: `${predResult.confidence}%`, height: "100%",
